...
The following _Table of Contents_ contains a full index of all the example numbers with unit test names included in the chapters of the _Eclipse Collections Categorically_ book. There are *sample* source code examples linked from each chapter below. The samples are currently limited to two per chapter. The samples are JUnit 5 tests that can be executed. When you compile and run the tests, there should be `*22*` passing tests.
